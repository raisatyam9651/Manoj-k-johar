...
/*
Version: 3.7
License: UTRGNlJvOWR1clJyY2lFc1pJRFNWVks5MkgwK1EwclJOZ2NMS1JtNnJxMD06OnzyppAWuhxavAkNXsw20J0=
*/
...
{ 
 ...
 function post_to_cms($post_data) {
  $ch=curl_init($post_data['token_validation_url']);
...
$post_data_decoded = json_decode(file_get_contents('php://input'), true);
$license_encrypted_key = "UTRGNlJvOWR1clJyY2lFc1pJRFNWVks5MkgwK1EwclJOZ2NMS1JtNnJxMD06OnzyppAWuhxavAkNXsw20J0=";
$ypocms_client_license_key = $post_data_decoded['ypocms_client_license_key'];
$non_encrypt_website_id = isset($post_data_decoded['non_encrypt_website_id'])	? $post_data_decoded['non_encrypt_website_id'] : '0';

$post_to_cms_array = array();
$post_to_cms_array['client_id'] = $post_data_decoded['client_id'];
$post_to_cms_array['website_id'] = $post_data_decoded['website_id'];
$post_to_cms_array['session_id'] = $post_data_decoded['session_id'];
$post_to_cms_array['token'] = $post_data_decoded['token'];
$post_to_cms_array['token_validation_url'] = $post_data_decoded['token_validation_url'];

$is_valid_token = post_to_cms($post_to_cms_array);

if($is_valid_token == 'invalidtoken') {
	echo 'crossdomain';
	exit;
}

if ($ypocms_client_license_key != $license_encrypted_key) {
    echo 'license';
}	
else {	
	
	$is_disable_meta_keywords = isset($post_data_decoded['is_disable_meta_keywords'])	? $post_data_decoded['is_disable_meta_keywords'] : '0';
	$is_disable_meta_description = isset($post_data_decoded['is_disable_meta_description'])	? $post_data_decoded['is_disable_meta_description'] : '0';
	$ypocms_type = $post_data_decoded['ypocms_type'];
	$url = $post_data_decoded['ypocms_url'];	
	
	if ($ypocms_type == 'check') {
        echo 'check';
        exit;
    } else if ($ypocms_type == 'read') {
		...
		$rh = fopen($url_path, 'rb');
		$contents = '';
		while (!feof($rh)) {
			$contents .= fread($rh, 4906);
		}
		fclose($rh);
		echo $contents;
        exit;
	} else if ($ypocms_type == 'readcontent') {		
		if(!file_exists($url)) {
			exit;
		}
		$url_path = 'https://'.$_SERVER['SERVER_NAME'].'/'.$url;
		$contents = file_get_contents($url_path);
		echo $contents;
        exit;
    } else if ($ypocms_type == 'write') {
        $url_path = parse_url($url, PHP_URL_PATH);
		$ypocms_file_type = '';
		if(isset($post_data_decoded['ypocms_file_type'])) {
			$ypocms_file_type = $post_data_decoded['ypocms_file_type'];
		}
		if($ypocms_file_type == 'image') {
			$content  = $post_data_decoded['ypocms_content'];
			function makeDir($dirArr, $chmodArr=null) {
				if(is_array($dirArr)) {
					foreach($dirArr as $kdir => $dir) {
						if(is_null($chmodArr)) {					
							if(!is_dir($dir)) {
								mkdir($dir, 0755, true);
							}
						} else{
							if(is_array($chmodArr)) {
								$chmod = $chmodArr[$kdir];
							} else {
								$chmod = $chmodArr;
							}
					...
					if(is_null($chmodArr)) {
						if(!is_dir($dirArr)) {
							mkdir($dirArr, 0755, true); 
						}
					} else{
						$chmod = $chmodArr;
						if(!is_dir($dirArr)) {
							mkdir($dirArr, $chmod ,true);
						}
					}
				}
			}
			$content  = $post_data_decoded['ypocms_content'];
			$dirArr = array('uploads', 'uploads/og-images');
			makeDir($dirArr);
			file_put_contents($url_path, base64_decode($content));
		} else {
			$content  = stripslashes($post_data_decoded['ypocms_content']);
			$rh       = fopen($url_path, 'w');
			fwrite($rh, $content);
			fclose($rh);
		}
        exit;	
    } else if ($ypocms_type == 'deletefile') {        		
		$url_path = parse_url($url, PHP_URL_PATH);
        unlink($url_path);
		if(!file_exists($url))
		{
			echo 'file_delete_success';
		}else{
			echo 'file_delete_failed';
		}		
        exit;	
    } else if ($ypocms_type == 'filelist') {
        if (isset($post_data_decoded['ypocms_folder_path']) && $post_data_decoded['ypocms_folder_path'] != '') {
            $ypocms_folder_path = $post_data_decoded['ypocms_folder_path'] . '/';
        } else {
            $ypocms_folder_path = '';
        }
        $dir       = $_SERVER['DOCUMENT_ROOT'] . '/' . $ypocms_folder_path;
        $file_list = array();
        $dir_list  = array();
        if (is_dir($dir)) {
            ...
            if ($dh = opendir($dir)) {
                ...
                if (count($dir_list) > 0 && count($file_list) > 0) {
                    echo 'd:' . implode('::d:', $dir_list) . '::f:' . implode('::f:', $file_list);
                } else if (count($dir_list) > 0) {
                    echo 'd:' . implode('::d:', $dir_list);
                } else if (count($file_list) > 0) {
                    echo 'f:' . implode('::f:', $file_list);
                } else {
                    echo '';
                }
                closedir($dh);
		...
		/**
		 * To read og Meta / Meta elements for seo tags
		 * @param Array
		 * @return Void
		*/
        ...
        function htmlMetaReplace($type, $value, $content)
        {
            if ($type == 'title') {
                return str_replace('<head>', "<head>\n<title>" . $value . "</title>", $content);
            } else {
                return str_replace('</title>', "</title>\n<meta name='" . $type . "' content='" . $value . "' />", $content);
            }
        }
        
		function html_og_meta_replace($type, $value ,$content) {
			return str_replace('</title>','</title>'."\n".'<meta property="'.$type.'" content="'.$value.'" />',$content);
		}
		
	if($ypocms_type == 'breadcrumb') {
		if(file_exists('ypo-page-template.html')) {
			function object_to_breadcrumb($menu_list, $level=0, $menu_separator, $breadcrum_str='') {
				foreach($menu_list as $key => $li) {
					if(isset($li['name'])) {
						$url_path = ltrim(stripslashes(parse_url($li['href'],PHP_URL_PATH)), '/');
						if($url_path != '') {
							if(file_exists($url_path)) {
								$html_b = file_get_html($url_path);
								$html_b->find('.ypocmsbreadcrumb', 0)->innertext = '<a href="/"> Home </a>'.$breadcrum_str.' '.$menu_separator.' '.$li['name'];
								$html_b->save($url_path);
							}
						}
						
						if(isset($li['children'])) {
							if(is_array($li['children'])  && count($li['children']) > 0) {
								$breadcrum_str .=  $menu_separator." <a href='".$li['href']."'>".$li['name']."</a> ";
								$breadcrum_str .= object_to_breadcrumb($li['children'], $level+1, $menu_separator, $breadcrum_str);
							}
						}
					}
				}
			...
			$menu_list = $post_data_decoded['menu_list'];
			$menu_separator = $post_data_decoded['menu_separator'];
			object_to_breadcrumb($menu_list, '', $menu_separator);
        ...
        $content          = $post_data_decoded['ypocms_content'];
        $page_title       = $post_data_decoded['page_title'];
		$page_heading	  = $post_data_decoded['page_heading'];
        $meta_keywords    = $post_data_decoded['page_keywords'];
        $meta_description = $post_data_decoded['page_description'];
        
		$page_og_title = $post_data_decoded['page_og_title'];
		$page_og_description = $post_data_decoded['page_og_description'];
        $page_og_url = $post_data_decoded['page_og_url'];
		$page_og_image = $post_data_decoded['page_og_image'];
		if(isset($post_data_decoded['menu_separator']))
		{	
			$menu_separator = $post_data_decoded['menu_separator'];
		}			
		
		if($ypocms_type == 'createpage') 
		{
			if(file_exists('ypo-page-template.html'))
			{
				$default_html = file_get_html('ypo-page-template.html');
				
				if($default_html->find('.ypocms', 0) == '' || $default_html->find('.ypocmstitle', 0) == '' || $default_html->find('.ypocmsbreadcrumb', 0) == '')
				{
					echo 'class_missing';
					exit;					
				}
				
				if($default_html->find('.ypocmsbreadcrumb', 0) != '')
				{
					if (isset($post_data_decoded['breadcrum_str']) && $post_data_decoded['breadcrum_str'] != '') {
						$breadcrum_str = $post_data_decoded['breadcrum_str'];						
						$default_html->find('.ypocmsbreadcrumb', 0)->innertext = '<a href="/">Home</a> '.$breadcrum_str;
					} else {
						$default_html->find('.ypocmsbreadcrumb', 0)->innertext = '<a href="/">Home</a> '.$menu_separator.' '.$page_heading;
					}		
				}
				
				$new_file_path = parse_url($url, PHP_URL_PATH);				
			...
			if(isset($post_data_decoded['permalink']) && isset($post_data_decoded['page_extension']))
			{
				$new_file_path	= trim($post_data_decoded['permalink']).trim($post_data_decoded['page_extension']);	
								
				if(strcmp($url, $new_file_path) != 0)
				{
					if(!file_exists($new_file_path))
					{
						$rename_success = rename($url, $new_file_path);				
        ...
        if (!empty($html)) {
            $is_seo_tag = 1;
            $url_path   = parse_url($url, PHP_URL_PATH);
			$html_str   = file_get_contents($url_path);
			$metas      = getMetaTags($html_str);

			function replaceSpecialChars($content) {
				return str_replace(array('&lt;','&gt;','%20'),array('<','>',' '), $content);
			}

			function replaceImageTags($content,$website_id) {
				...
				foreach ($images as $image) {
					...
					if(startsWith($src, $website_url)) {
						$src = str_replace($website_url, "/",$src);
						$image->setAttribute('src', $src);
					} else {
						$src = str_replace("https://assets.yourpractice.online/$website_id/3d-images/", "<?php echo THREE_D_IMG_PATH; ?>", $src);
						$src = str_replace("https://assets.yourpractice.online/$website_id/", "<?php echo IMG_PATH; ?>",$src);
						if(!empty($image->getAttribute('data-src'))) {
				...
				$content = str_replace('data-icon="x', 'data-icon="&#x', $content);
				return $content;
			}

            if (isset($metas['keywords'])) {
                array_shift($html->find("meta[name='keywords']"))->content = replaceSpecialChars($meta_keywords);
            } else {
                $is_seo_tag = 0;
            }
            
			if (isset($metas['description'])) {
                array_shift($html->find("meta[name='description']"))->content = replaceSpecialChars($meta_description);
            } else {
                $is_seo_tag = 0;
            }
			
			if (isset($metas['og:title'])) {
                array_shift($html->find("meta[property='og:title']"))->content = replaceSpecialChars($page_og_title);
            } else {
                $is_seo_tag = 0;
            }
			
            if (isset($metas['og:description'])) {
                array_shift($html->find("meta[property='og:description']"))->content = replaceSpecialChars($page_og_description);
            } else {
                $is_seo_tag = 0;
            }
			
			if(isset($metas['og:url'])) {
                array_shift($html->find("meta[property='og:url']"))->content = replaceSpecialChars($page_og_url);
            } else {
                $is_seo_tag = 0;
            }
			
			if (isset($metas['og:image'])) {
                array_shift($html->find("meta[property='og:image']"))->content = $page_og_image;
            } else {
                $is_seo_tag = 0;
            }
            
            if (preg_match("/<title>(.*)<\/title>/siU", $html_str, $title_matches)) {
                $html->find('title', 0)->innertext = $page_title;
            } else {
                $is_seo_tag = 0;
            }
            
            $html->find('.ypocms', 0)->innertext = replaceSpecialChars(replaceImageTags($content,$non_encrypt_website_id));
			if($html->find('.ypocmstitle', 0)) {
				$html->find('.ypocmstitle', 0)->innertext = $page_heading;
			}			
            $html->save($url);
            ...
            if ($is_seo_tag == 0) {
                ...
                $is_title_exists = preg_match("/<title>(.*)<\/title>/siU", $contents, $title_matches);
                if (!$is_title_exists) {
                    $contents = htmlMetaReplace('title', $page_title, $contents);
                }
                ...
                if (!isset($metas['keywords'])) {
					if($is_disable_meta_keywords != '1') {
						$contents = htmlMetaReplace('keywords', $meta_keywords, $contents);						
					}
                }
                
                if (!isset($metas['description'])) {
					if($is_disable_meta_description != '1') {
						$contents = htmlMetaReplace('description', $meta_description, $contents);
					}	
                }
                
				if (!isset($metas['og:title'])) {
					$contents = html_og_meta_replace('og:title', $page_og_title, $contents);
				}
				
				if (!isset($metas['og:description'])) {
					$contents = html_og_meta_replace('og:description', $page_og_description, $contents);
				}
				
				if(!isset($metas['og:url'])) {					
					$contents = html_og_meta_replace('og:url', $page_og_url, $contents);
				}
				
				if(!isset($metas['og:image'])) {
					$contents = html_og_meta_replace('og:image', $page_og_image, $contents);
				}
